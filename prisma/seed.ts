import { PrismaClient, NewsCategory, NewsStatus, GalleryCategory, Terms } from '../src/generated/prisma';
...
async function main() {
  ...
  // Create Classes (now including schoolid)
  ...
  // Subjects & link to teacher via teacherid
  const subjectData = [
    ...
    ...d,
    schoolid: school.id,
    teacherid: teacher.id,      // ← assign the teacher here
    createdAt: new Date(),
    updateAt: new Date(),
  }));

  // bulk-create (skipping any duplicates by name + schoolid + teacherid)
  await prisma.subject.createMany({
    data: subjectData,
    skipDuplicates: true,
  });

  // (Optional) if you need the full records afterward:
  ...
  // Attendance (past 7 days, skip weekends)
  ...
  // Current Term (now with schoolid)
