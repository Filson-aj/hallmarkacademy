...
const Parents: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            show("error", "Fetch Error", "Failed to fetch parents, please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmDelete = useCallback((ids: string[]) => {
        confirmDialog({
            ...
            accept: async () => {
                ...
                try {
                    await deleteApi(ids);
                    show("success", "Deleted", ids.length === 1 ? "Parent deleted successfully." : `${ids.length} parents deleted successfully.`);
                    setParents(prev => prev.filter(t => !ids.includes(t.id)));
                    setSelected(prev => prev.filter(t => !ids.includes(t.id)));
                } catch (err: any) {
                    show("error", "Deletion Error", err.message || "Failed to delete parent(s).");
                } finally {
                    setDeletingIds([]);
                }
            }
    ...
    const handleUpdate = useCallback((updated: any) => {
        setParents(prev => prev.map(t => t.id === updated.id ? updated : t));
        setEdit(false);
    }, []);

    const handleNewParent = useCallback((newParent: any) => {
    ...
    return (
        <section className="flex flex-col w-full py-3 px-4">
            ...
            {view && <Parent parent={current} visible={view} onClose={() => setView(false)} />}
            {create && <NewParent close={() => setCreate(false)} onCreated={handleNewParent} />}
            {edit && current && (
                <EditParent parent={current} close={() => setEdit(false)} onUpdated={handleUpdate} />
            )}
            ...
            <div className="bg-white rounded-md shadow-md space-y-4">
                <div className="flex justify-between items-center border-b border-gray-200 px-3 py-2">
                    <h1 className="text-2xl font-bold text-gray-700">All Parents</h1>
                ...
                >
                    ...
                    <Column
                        header="Children"
                        body={(rowData) => rowData.students?.length || 0}
                    />
